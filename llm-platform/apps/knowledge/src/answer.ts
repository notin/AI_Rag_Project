...
const PROMPT_VERSION = 'answer@v4';
...
// `describe` is injected into the JSON schema `generateObject` sends, so
// length belongs here — a note tacked onto the question line loses to
// "keep it concise" in the system prompt and to a one-line field description.
const AnswerSchema = z.object({
  answer: z
    .string()
    .describe(
      '2–3 short grounded paragraphs answering the question from the context (direct answer first, then supporting detail). Use the exact decline sentence only when the context cannot answer.',
    ),
  citations: z
    .array(z.string())
    .describe(
